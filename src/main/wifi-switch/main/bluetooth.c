...
#define TEST_DEVICE_NAME            "SmartPlug v1.0"
...
//		0x0f, 0x09, 0x53, 0x6d, 0x61, 0x72, 0x74, 0x50, 0x6c, 0x75, 0x67, 0x20, 0x76, 0x31, 0x2e, 0x30
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_REG_EVT:
		...
		name[0] = 0;
		int i= 0;
		while ( conf->intName[i] >= 32 && conf->intName[i] <= 122 && i < 65 )
		{
			name[i] = conf->intName[i];
			name[++i] = 0; 
		}
		
		if ( strlen( name ) == 0 )
			strcpy( name, "SmartSwith1.0" );
...
static void gatts_profile_b_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (!param->write.is_prep){
			...
			{
				uint8_t mark = param->write.value[0];

				if ( mark == 0x6f )
				{
					uint8_t index = param->write.value[1];
					uint8_t len = param->write.value[2];
					
					if ( index == 0 )
						strcpy( setupCommand, "" );
				
					strncat( setupCommand, (const char *)&param->write.value[3], len );
					ESP_LOGI(GATTS_TAG, "part[%d]=%s", index, (const char *)&param->write.value[3] );
				}
				else if ( mark == 0x7F )
				{
					set_configuration( setupCommand );
				}
				else
					ESP_LOGI(GATTS_TAG, "Unknown mark %d", mark );
			}
...
void bluetooth_config_task(void *pvParameters)
{
    esp_err_t ret;
